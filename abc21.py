...
user_input=st.text_input("Enter the desired email template's keyword")
...
def search_similar_temp(search_term):
    ...
    return pd.DataFrame(temp_names, columns=['template Context','email template Title','template Subject','Accuracy'])
...
if (st.button("Press here")):
    result1=search_similar_temp(user_input)
